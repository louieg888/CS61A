...
CREATE TABLE schedule AS
  WITH options(totalPrice, list, flightsLeft, lastDestination) AS (
    SELECT price, departure || ", " || arrival, 1, arrival FROM flights WHERE departure = "SFO" UNION
    SELECT totalPrice + price, list || ", " || arrival, flightsLeft - 1, arrival FROM flights, options WHERE lastDestination = departure AND flightsLeft > 0
  )
  SELECT list, totalPrice from options WHERE lastDestination = "PDX" ORDER BY totalPrice;

CREATE TABLE number_of_options AS
  SELECT COUNT(DISTINCT meat) from main_course;

CREATE TABLE calories AS
  SELECT COUNT(*) from main_course AS main, pies AS pie
...
CREATE TABLE shopping_list AS
  ...
  SELECT items.item AS myItem, lowest_prices.store AS store FROM lowest_prices, items
    WHERE lowest_prices.item = items.item
    GROUP BY items.item;

CREATE TABLE total_bandwidth AS
  WITH mbsTable(mb) AS (
    SELECT SUM(Mbs) FROM shopping_list, stores WHERE stores.store = shopping_list.store GROUP BY shopping_list.myItem
  )
  SELECT SUM(mb) FROM mbsTable;
