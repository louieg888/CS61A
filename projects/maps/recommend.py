...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    clusterList = {repr(centroid) : [] for centroid in centroids}
    ...
    for restaurant in restaurants:
        ...
        closestCentroid = find_closest(location, centroids)
        # add the location to the array of current clusters
        closestCentroidLocation = repr(closestCentroid)
        clusterList[closestCentroidLocation].append(restaurant)

    # remove empty clusters
    returnVal = [x for x in list(clusterList.values()) if x != []]
    return returnVal
...
def find_centroid(cluster):
    ...
    zippedLocations = list(zip(*locations))
    averageLocation = [mean(coordinates) for coordinates in zippedLocations]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            restaurantReviews[name] = user_rating(user, name)
        else:
            restaurantReviews[name] = predictor(restaurant)
